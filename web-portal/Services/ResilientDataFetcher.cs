...
public class ResilientDataFetcher
{
		...
		int retryDelayMs = 500)
	{
		...
		catch (Exception ex)
		{
			...
			if (result.RetryAttempts < maxRetries)
			{
				...
				return await FetchAsync(fetchFunc, result, maxRetries, staleThresholdSeconds, retryDelayMs);
			}
		...
		int retryDelayMs = 500)
	{
		...
		return await FetchAsync(fetchFunc, currentResult, currentResult.MaxRetries, currentResult.StaleThresholdSeconds, retryDelayMs);
	}
}
